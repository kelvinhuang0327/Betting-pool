...
class MarketMistakeDetector:
    ...
    def scan_for_typos(self, game_id: str, odds: List[OddsLine], market_consensus: Dict[str, float]) -> List[MarketAnomaly]:
        ...
        for o in odds:
            ...
            if key not in market_consensus: continue
    ...
    def detect_arbitrage(self, game_id: str, odds: List[OddsLine]) -> List[MarketAnomaly]:
        ...
        for o in odds:
            m_key = o.market
            if m_key not in markets: markets[m_key] = {}
            if o.side not in markets[m_key]: markets[m_key][o.side] = []
            markets[m_key][o.side].append(o.decimal_odds)
            
        found = []
        for m, sides in markets.items():
            if len(sides) < 2: continue # Need at least 2 sides (e.g. Home vs Away)
        ...
        for t in tsl_odds:
            for s in sharp_odds:
                if t.market == s.market and t.side == s.side:
                    # Significant gap (TSL giving better value than a sharp book)
                    if t.decimal_odds > s.decimal_odds * (1.0 + self.stale_threshold):
