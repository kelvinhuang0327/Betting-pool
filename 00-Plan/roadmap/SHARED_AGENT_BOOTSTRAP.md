...
Use this precedence for non-safety instructions:
...
Safety rules are cumulative. A lower or broader instruction must never weaken a stricter rule.
...
* Staged files exist before the task and the task does not explicitly permit handling them.
* Unrelated dirty files create ambiguity.
...
* Limited code, test, documentation, or artifact changes.
* Allowed files must be explicit.
* No DB write, production write, registry mutation, or controlled apply unless separately authorized.
